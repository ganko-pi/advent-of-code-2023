...
Dictionary<Position, PartNumber> partNumbers = new();
// store numbers
int currentLengthOfCurrentParsedNumber = 0;
foreach ((string line, int indexY) in input.Split(Environment.NewLine).Select((x, i) => (x, i)))
{
    // reset on each new line
    currentLengthOfCurrentParsedNumber = 0;

    foreach ((char character, int indexX) in line.Select((x, i) => (x, i)))
    {
        ...
        // initialize with current value which is overwritten if the previous positions already had a number
        PartNumber currentPartNumber = new((int)char.GetNumericValue(character));
        if (currentLengthOfCurrentParsedNumber > 0)
        {
            // update currentPartNumber
            Position positionToUpdate = new(indexX - 1, indexY);
            currentPartNumber = partNumbers[positionToUpdate];
            // update all references
            currentPartNumber.Number = currentPartNumber.Number * 10 + (int)char.GetNumericValue(character);
        }

        Position currentPosition = new(indexX, indexY);
        // add new number if previous position had no number or add the same reference as the previous positions
        partNumbers.Add(currentPosition, currentPartNumber);

        ++currentLengthOfCurrentParsedNumber;
    }
}
...
foreach ((string line, int indexY) in input.Split(Environment.NewLine).Select((x, i) => (x, i)))
{
    foreach ((char character, int indexX) in line.Select((x, i) => (x, i)))
    {
        ...
        HashSet<PartNumber> adjacentPartNumbersForGear = new();
        foreach (Position adjacentPosition in adjacentPositions)
        {
            if (!partNumbers.ContainsKey(adjacentPosition))
            {
                continue;
            }
            
            adjacentPartNumbersForGear.Add(partNumbers[adjacentPosition]);
        }

        if (adjacentPartNumbersForGear.Count != 2)
        {
            continue;
        }

        foreach (PartNumber partNumber in adjacentPartNumbersForGear)
        {
            partNumber.HasAdjacentGear = true;
            partNumber.RelatedPartNumbers = adjacentPartNumbersForGear.Where(pn => pn != partNumber).ToList();
        }
    }
}

// multiply and sum numbers adjacent to parts
int sum = 0;
foreach (PartNumber partNumber in partNumbers.Values)
{
    if (!partNumber.HasAdjacentGear)
    {
        continue;
    }

    if (partNumber.AlreadyInSum)
    {
        continue;
    }

    foreach (PartNumber relatedPartNumber in partNumber.RelatedPartNumbers)
    {
        if (relatedPartNumber.AlreadyInSum)
        {
            continue;
        }

        sum += partNumber.Number * relatedPartNumber.Number;
    }

    partNumber.AlreadyInSum = true;
}
