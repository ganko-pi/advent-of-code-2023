...
int allowedMaxRed = 12;
int allowedMaxGreen = 13;
int allowedMaxBlue = 14;

int sum = 0;
foreach (string line in input.Split(Environment.NewLine))
{
    string[] gamesAndSets = line.Trim().Split(':');
    if (gamesAndSets.Length == 1)
    {
        continue;
    }

    int gameId = int.Parse(gamesAndSets[0].Trim().Split(' ')[1]);
    string[] sets = gamesAndSets[1].Trim().Split(';');
    ...
    {
        string[] setContents = set.Trim().Split(',');
        foreach (string setContent in setContents)
        {
            string[] amountAndColor = setContent.Trim().Split(' ');
            int amount = int.Parse(amountAndColor[0]);
            switch (amountAndColor[1])
            {
                case "red":
                    if (foundMaxRed < amount)
                    {
                        foundMaxRed = amount;
                    }
                    break;
                case "green":
                    if (foundMaxGreen < amount)
                    {
                        foundMaxGreen = amount;
                    }
                    break;
                case "blue":
                    if (foundMaxBlue < amount)
                    {
                        foundMaxBlue = amount;
                    }
                    break;
    ...
    if (foundMaxRed <= allowedMaxRed
        && foundMaxGreen <= allowedMaxGreen
        && foundMaxBlue <= allowedMaxBlue)
        {
            sum += gameId;
        }
}
