...
public record Hand : IComparable<Hand>
{
    ...
    )
    {
        ...
        if (countPerLabel.ContainsKey(Label.L_J))
        {
            int countOfJokers = countPerLabel[Label.L_J];
            if (countOfJokers == 5)
            {
                return Type.FIVE_OF_A_KIND;
            }

            countPerLabel.Remove(Label.L_J);
            Label labelWithMaxOccurences = countPerLabel.MaxBy(keyValuePair => keyValuePair.Value).Key;
            countPerLabel[labelWithMaxOccurences] += countOfJokers;
        }
